import { Clock, Edit2, Plus, Sparkles, Utensils, X } from "lucide-react";
...
export function MealDetailModal({
	...
	const calculateGrandTotals = () => {
		return allMeals.reduce(
			(grandTotal, mealRecord) => {
				if (
					mealRecord.isMultipleMeals &&
					mealRecord.meals &&
					mealRecord.meals.length > 0
				) {
					const mealTotals = mealRecord.meals.reduce(
						(total: any, individualMeal: any) => ({
						...
						{allMeals.map((mealRecord, recordIndex) => (
							...
							>
										...
										<div className="w-20 h-20 flex-shrink-0">
											{mealRecord.images && mealRecord.images.length > 0 ? (
												...
												mealRecord.image && (
													<button
														onClick={() =>
															handleImageClick([mealRecord.image], 0)
														}
														className="w-full h-full"
													>
														<ImageWithFallback
								...
								<div className="space-y-2">
									...
									mealRecord.meals.length > 0 ? (
										// 複数食事：個別カード表示
										mealRecord.meals.map(
											(individualMeal: any, index: number) => (
												<Card
													key={index}
