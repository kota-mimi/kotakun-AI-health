...
export function MealAnalysisPage({
	...
	const [trendPeriod, setTrendPeriod] = useState<"7days" | "14days" | "30days">(
		"7days",
	);
	...
	const calculateDailyNutrition = () => {
		...
		const totalFat = allMeals.reduce(
			(sum: number, meal: any) => sum + (meal.fat || 0),
			0,
		);
		...
		const lastMealTime =
			mealTimes.length > 0 ? mealTimes[mealTimes.length - 1] : null;
	...
	// 達成率計算
	const calorieProgress = Number(
		((nutrition.calories / nutrition.targets.calories) * 100).toFixed(1),
	);
	const proteinProgress = Number(
		((nutrition.protein / nutrition.targets.protein) * 100).toFixed(1),
	);
	const fatProgress = Number(
		((nutrition.fat / nutrition.targets.fat) * 100).toFixed(1),
	);
	const carbsProgress = Number(
		((nutrition.carbs / nutrition.targets.carbs) * 100).toFixed(1),
	);

	// トレンド計算
	const yesterdayCalories =
		weeklyTrends[weeklyTrends.length - 2]?.calories || 0;
	const calorieChange = nutrition.calories - yesterdayCalories;
	const caloriesTrend =
		calorieChange > 0 ? "up" : calorieChange < 0 ? "down" : "stable";
	...
	return (
		<div className={hideHeader ? "min-h-screen bg-white relative" : ""}>
			...
			{hideHeader && (
				<div
					className="absolute inset-0 bg-gradient-to-br from-blue-100/30 via-transparent to-indigo-50/20 pointer-events-none"
					style={{
						background:
							"linear-gradient(135deg, rgba(70, 130, 180, 0.1) 0%, transparent 50%, rgba(70, 130, 180, 0.05) 100%)",
					}}
				></div>
			)}
			...
			<div className={`space-y-4 ${hideHeader ? "" : "px-4 py-4"}`}>
				...
				<Card className=" bg-white shadow-sm border border-gray-200 rounded-xl p-2">
					<div className="flex items-center justify-between mb-2">
						...
						<Button
							...
							size="sm"
							onClick={() =>
								onNavigateToNutritionSettings && onNavigateToNutritionSettings()
							}
					...
					<div className="space-y-2">
						...
						{nutritionOptions
							.filter(
								(option) =>
									selectedNutrients[
										option.key as keyof typeof selectedNutrients
									],
							)
							.map((option) => {
								const currentValue =
									option.key === "protein"
										...
										: option.key === "fat"
											...
											: option.key === "carbs"
												? nutrition.carbs
												: nutrition.extended[
														option.key as keyof typeof nutrition.extended
													];
								const targetValue =
									nutrition.targets[
										option.key as keyof typeof nutrition.targets
									];
								...
								return (
									<div key={option.key}>
										...
										<div className="flex justify-between">
											...
											<span
												className={`text-xs ${getProgressStatus(progress).color}`}
											>
						...
						{/* 選択された栄養素がない場合 */}
						{Object.values(selectedNutrients).every(
							(selected) => !selected,
						) && (
				...
				{/* 記録した食事内容 */}
				<div>
					<h3 className="font-semibold text-slate-800 mb-4 px-4">
						本日の食事記録
					</h3>
					<InstagramLikeFeed
						mealData={mealData}
