...
export function PlanSettingsPage({ onBack }: PlanSettingsPageProps) {
  ...
  useEffect(() => {
    const fetchCurrentPlan = async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/user/current-plan?userId=${liffUser.userId}`);
        
        if (response.ok) {
          const data = await response.json();
          console.log('📊 Current plan API response:', data);
          
          if (data.success && data.currentPlan) {
            setCurrentPlan(data.currentPlan); // 'monthly', 'quarterly', 'free'
            console.log('✅ Current plan set to:', data.currentPlan);
          } else {
            setCurrentPlan('free');
            console.log('⚠️ No current plan found, defaulting to free');
          }
        } else {
          console.log('❌ Current plan API failed with status:', response.status);
          setCurrentPlan('free');
        }
      } catch (error) {
  ...
  const plans = [
    ...
    },
    {
      ...
      monthlyPrice: 600,
      discount: '32%OFF',
      stripePriceId: 'price_1SNPADKMirzoVNsdLmsBPZ3o', // 新しいStripe価格ID (¥1,800)
      isRecommended: true,
