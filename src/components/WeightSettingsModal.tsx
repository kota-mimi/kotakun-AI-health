...
export function WeightSettingsModal({
	...
	const [targetWeight, setTargetWeight] = useState(
		currentSettings.targetWeight.toString(),
	);
	...
	const adjustTarget = (adjustment: number) => {
		const currentValue =
			parseFloat(targetWeight) || currentSettings.targetWeight;
				...
				<form onSubmit={handleSubmit} className="space-y-6">
					...
					<div className="space-y-3">
						<Label className="flex items-center space-x-2 text-slate-700">
							...
							<span>
								目標期限 <span className="text-xs text-slate-500">任意</span>
							</span>
						</Label>
						<Input
							type="date"
