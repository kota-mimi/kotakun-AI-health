...
export function AddMealModal({
	...
	const getAllPastMeals = () => {
		...
		Object.entries(allMealsData).forEach(
			([mealType, meals]: [string, any[]]) => {
				meals.forEach((meal) => {
					allMeals.push({
						...
						mealType,
						date: meal.createdAt
							? new Date(meal.createdAt).toLocaleDateString("ja-JP")
							: "日付不明",
			...
			try {
				...
				if (response.ok) {
					...
					if (analysis.isMultipleMeals && analysis.meals) {
						...
						setMealName(
							analysis.meals
								.map((meal: any) => meal.displayName || meal.name)
								.join("、"),
						);
		...
		try {
			...
			if (response.ok) {
				...
				if (analysis.isMultipleMeals && analysis.meals) {
					...
					// 単一食事の場合
					setMealName(
						analysis.displayName || analysis.foodItems?.[0] || textInput,
					);
					setFoodItems([
						{
							id: generateId(),
			...
			// フォールバック - 基本的なデータ
			setMealName(
				textInput.length > 20 ? textInput.substring(0, 20) + "..." : textInput,
			);
			setFoodItems([
				{
					id: generateId(),
	...
	const handleSubmit = () => {
		...
		if (foodItems.length > 1 && onAddMultipleMeals) {
			...
			// 単一食事または手動入力の場合
			const totalCalories =
				foodItems.length > 0 ? foodItems[0].calories : parseInt(calories) || 0;

			const totalProtein =
				foodItems.length > 0 ? foodItems[0].protein : parseInt(protein) || 0;

			const totalFat =
				foodItems.length > 0 ? foodItems[0].fat : parseInt(fat) || 0;

			const totalCarbs =
				foodItems.length > 0 ? foodItems[0].carbs : parseInt(carbs) || 0;
