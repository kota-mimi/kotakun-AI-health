...
export function WeightChart({
	...
	const counselingInitialData = useMemo(() => {
		...
		const counselingDateRaw =
			counselingResult.completedAt || counselingResult.createdAt;
		let counselingDate;
	...
	const chartData = useMemo(() => {
		...
		const processedData = downsampledData
			.map((item) => {
				...
				if (isNaN(itemDate.getTime())) {
					if (process.env.NODE_ENV === "development") {
						console.warn(
							"⚠️ Invalid date in WeightChart processedData:",
							item.date,
							"parsed as:",
							itemDate,
						);
	...
	const dataRange = useMemo(() => {
		...
		if (values.length === 0) {
			...
			switch (dataType) {
				case "weight":
					return { min: 70, max: 77 };
				case "waist":
					return { min: 77, max: 87 };
			}
		}
	...
	const pathPoints = useMemo(() => {
		...
		return currentData.map((point, index) => {
			...
			const y =
				10 +
				(svgHeight - 25) -
				((point.value - currentConfig.min) / chartRange) * (svgHeight - 25);
	...
	const createAreaPath = (points: typeof pathPoints) => {
		...
		const allSameValue =
			points.length >= 2 &&
			points.every((p) => Math.abs(p.value - points[0].value) < 0.01);
	...
	const targetY = targetValue
		? 10 +
			(svgHeight - 25) -
			((targetValue - currentConfig.min) / chartRange) * (svgHeight - 25)
		: null;
	...
	const generateYAxisTicks = () => {
		...
		// 開始値を間隔に合わせて調整
		const startValue =
			Math.floor(currentConfig.min / tickInterval) * tickInterval;
		...
		for (let value = startValue; value <= endValue; value += tickInterval) {
			if (value >= currentConfig.min && value <= currentConfig.max) {
				const normalizedY =
					(value - currentConfig.min) / (currentConfig.max - currentConfig.min);
	...
	const handleMouseMove = useCallback(
		(event: React.MouseEvent<SVGElement>) => {
			...
			const endPoint =
				pathPoints[Math.min(segmentIndex + 1, pathPoints.length - 1)];

			// 線形補間でY座標を計算
			const t =
				startPoint.x === endPoint.x
					? 0
					: (relativeX - startPoint.x) / (endPoint.x - startPoint.x);
			const interpolatedY = startPoint.y + (endPoint.y - startPoint.y) * t;
			const interpolatedValue =
				startPoint.value + (endPoint.value - startPoint.value) * t;
					...
					<div className="relative">
						<div className="relative">
							...
							>
								...
								{/* X軸ラベル - 期間に応じて適切に表示 */}
								{(() => {
									const filteredPoints = pathPoints.filter(
