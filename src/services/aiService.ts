...
true または false で回答してください。`;
  ...
  async generateRecipeWithFlex(userMessage: string, userId?: string): Promise<{ textResponse: string; flexMessage?: any }> {
    try {
      ...
      // ランダムジャンル・料理スタイルを追加してバリエーション強化
      const cuisineTypes = ['和風', '洋風', 'アジア風', '中華風', 'イタリアン', '韓国風', 'タイ風', 'メキシカン', '地中海風', 'インド風'];
      const cookingStyles = ['蒸し', '焼き', '煮込み', '炒め', 'グリル', '揚げない揚げ物風', 'サラダ風', 'スープ', '丼もの', 'パスタ風'];
      const mainIngredients = ['魚', '鶏肉', '豆腐', '卵', 'きのこ', '豆類', '野菜メイン', '海鮮', 'チーズ', 'ナッツ'];
...
- 断固禁止ワード：「やあ」「〇〇さ」「〇〇だとか」「〇〇とか」「〇〇ですとか」「〇〇みたいな」「〇〇ような」「なんて」「って感じで」「かもしれません」「だと思います」「でしょうか」「だが」「しかし」「である」「のである」「おう」「おお」「おい」「だぜ」「〜ぜ」
...
**絶対禁止ワード**：「だが」「しかし」「である」「のである」「おう」「おお」「おい」「だぜ」「〜ぜ」などの古風・不自然・荒っぽい表現は一切使わない。
