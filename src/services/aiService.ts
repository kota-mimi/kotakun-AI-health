...
`;
  ...
  async isRecipeQuestion(userMessage: string): Promise<boolean> {
    try {
      ...
      const prompt = `
以下のメッセージが**新しいレシピや料理の作り方を求める質問**かどうか判定してください。
...
- 「○○の作り方教えて」「○○のレシピを教えて」
- 「○○ってどうやって作るの？」「○○作りたい」
- 「ヘルシーな○○の作り方」「ダイエット用の○○レシピ」
- 「○○で作れるもの教えて」「安く作れる料理」
- 「冷蔵庫に○○があるから何か作りたい」

**レシピ要求ではない（false）：**
- 既存レシピへの質問・提案：「ごま油かけてもいい？」「塩多めでも大丈夫？」「これってヘルシー？」
- 栄養・カロリー質問：「このレシピのカロリーは？」「タンパク質どれくらい？」
- アレンジ質問：「このレシピを○○風にできる？」「代用できる調味料は？」
- 一般的な健康相談：「疲れた」「お腹すいた」「運動したい」
- 感想・評価：「美味しそう」「作ってみる」「ありがとう」

**重要**: 調味料・食材名が含まれていても、既存レシピへの質問・提案は false
