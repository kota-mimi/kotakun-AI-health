...
`;
  ...
  async isRecipeQuestion(userMessage: string): Promise<boolean> {
    try {
      ...
      const prompt = `
以下のメッセージが**料理やレシピに関する質問**かどうか判定してください。

メッセージ: "${userMessage}"

**レシピ・料理関連の質問（true）：**
- レシピ要求：「○○の作り方」「○○のレシピ」「○○作りたい」「○○ってどう作る？」
- 材料指定：「鶏肉で何作れる？」「冷蔵庫に○○があるから」「余った○○で」
- 条件指定：「簡単な料理」「ヘルシーな○○」「ダイエット向け」「安く作れる」「10分で作れる」
- 料理相談：「今日の晩ご飯何にしよう」「お昼何食べよう」「何作ろうかな」
- 食材活用：「○○使って」「○○消費したい」「○○が余ってる」
- 時短・簡単：「手軽な」「時短で」「簡単に」「すぐできる」
- 予算重視：「節約料理」「安い」「○円で」「コスパ良い」
- 栄養重視：「タンパク質多め」「野菜たっぷり」「低糖質」「高タンパク」
- 気分・欲求：「さっぱりしたもの」「こってりしたい」「温かいもの」「冷たいもの」
- 曖昧な要求：「何か美味しいもの」「おすすめ料理」「人気のレシピ」

**レシピ要求ではない（false）：**
- 運動・トレーニング：「筋トレ」「トレーニング」「運動」「ワークアウト」「エクササイズ」「ジム」
- 健康・ダイエット方法：「健康になる方法」「痩せる方法」「ダイエット方法」「体重を落とす方法」
- 既存レシピへの質問：「ごま油足してもいい？」「これヘルシー？」「カロリーどれくらい？」
- 一般相談：「疲れた」「眠い」「体調悪い」「具合悪い」
- 感想・評価：「美味しそう」「作ってみる」「ありがとう」「いいね」

**判定基準：**
- 少しでも「料理を作りたい」「何を食べるか考えたい」ニュアンスがあれば → true
- 料理名や食材名が入っていて、作る意図が感じられれば → true
- 曖昧でも食事・料理に関する相談なら → true
...
2. 簡単な情報質問（40-60文字で簡潔に）：
   - 「○○のカロリーなに？」→「○○は100gで○○kcal、P○g・F○g・C○g。筋肉づくりに効果的」

3. 中程度の相談（80-120文字で適度に）：
   - 筋トレ種目の質問
   - ダイエット方法
   - 食事のアドバイス

4. 真剣な相談・詳しい説明が必要（150-300文字で詳しく包括的に）：
   - 「本格的にダイエットしたい」
   - 「筋トレメニューを組んでほしい」
   - 「健康について悩んでる」
   - 「○○について詳しく教えて」
...
- 断固禁止ワード：「やあ」「〇〇さ」「〇〇だとか」「〇〇とか」「〇〇ですとか」「〇〇みたいな」「〇〇ような」「なんて」「って感じで」「かもしれません」「だと思います」「でしょうか」
