...
export function useMealData(
	...
	const [firestoreMealData, setFirestoreMealData] = useState<MealData>({
		breakfast: [],
		lunch: [],
		dinner: [],
		snack: [],
	...
	useEffect(() => {
		const fetchMealData = async () => {
			...
			if (!selectedDate || isNaN(selectedDate.getTime())) {
				if (process.env.NODE_ENV === "development") {
					console.warn(
						"⚠️ Invalid selectedDate in fetchMealData:",
						selectedDate,
					);
				}
				return;
			...
			try {
				...
				if (response.ok) {
					...
					if (data.success && data.mealData) {
						...
						const multipleMeals = allMeals.filter(
							(meal) => meal.isMultipleMeals,
						);
					...
					if (process.env.NODE_ENV === "development") {
						...
						// 複数食事が含まれているかチェック
						const allTestMeals = [
							...testMealData.breakfast,
							...testMealData.lunch,
							...testMealData.dinner,
							...testMealData.snack,
						];
						const multipleTestMeals = allTestMeals.filter(
							(meal) => meal.isMultipleMeals,
						);
	...
	const calculateDailyNutrition = () => {
		...
		const totalProtein =
			Math.round(
				allMeals.reduce((sum, meal) => sum + (meal.protein || 0), 0) * 10,
			) / 10;
		const totalFat =
			Math.round(
				allMeals.reduce((sum, meal) => sum + (meal.fat || 0), 0) * 10,
			) / 10;
		const totalCarbs =
			Math.round(
				allMeals.reduce((sum, meal) => sum + (meal.carbs || 0), 0) * 10,
			) / 10;

		// 日付ベースのプロフィール履歴から目標値を取得
		const targetValues = getTargetValuesForDate(
			dateProfileData,
			counselingResult,
		);
		...
		const carbsTarget = targetValues.macros.carbs;
	...
	const handleAddMealSubmit = async (meal: Omit<Meal, "id">) => {
		...
		try {
			...
			if (response.ok) {
				...
				alert(
					errorData.error ||
						"記録の制限に達しました。有料プランにアップグレードしてください。",
				);
				return;
			} else {
	...
	const handleAddMultipleMeals = async (meals: Omit<Meal, "id">[]) => {
		...
		if (!selectedDate || isNaN(selectedDate.getTime())) {
			if (process.env.NODE_ENV === "development") {
				console.warn(
					"⚠️ Invalid selectedDate in addMultipleMeals:",
					selectedDate,
				);
			}
			return;
	...
	const handleUpdateMeal = async (updatedMeal: Meal) => {
		...
		if (
			updatedMeal.originalMealId &&
			updatedMeal.individualMealIndex !== undefined
		) {
			...
			if (response.ok) {
				...
				updateDateData({
					mealData: {
						...
						[currentMealType]: currentData.mealData[currentMealType].map(
							(meal) => (meal.id === originalMealId ? updatedMeal : meal),
						),
					},
				});
	...
	const handleDeleteMeal = async (mealId: string) => {
		...
		try {
			...
			const optimisticUpdate = () => {
				if (individualMealIndex !== undefined) {
					...
					updateDateData({
						mealData: {
							...currentData.mealData,
							[currentMealType]: currentData.mealData[currentMealType].filter(
					...
					setFirestoreMealData((prev) => ({
						...prev,
						[currentMealType]: prev[currentMealType].filter(
							(meal) => meal.id !== mealId,
						),
					}));
				}
			...
			if (response.ok) {
				...
				console.error(
					"🚨 Production: Firestore delete failed, rolling back:",
					response.status,
				);
			...
			console.error(
				"🚨 Production: Meal deletion error, ensuring data consistency:",
				error,
			);
			...
			try {
				...
				console.error(
					"🚨 Production: Failed to restore data consistency:",
					syncError,
				);
			}
		}
	...
	const handleDeleteIndividualMeal = (
		...
		for (const mealType of allMealTypes) {
			const mealIndex = currentData.mealData[mealType].findIndex(
				(meal) => meal.id === originalMealId,
			);
			if (mealIndex !== -1) {
				const targetMeal = currentData.mealData[mealType][mealIndex];
				if (targetMeal.isMultipleMeals && targetMeal.meals) {
					// 該当する個別食事を削除
					const updatedMeals = targetMeal.meals.filter(
						(_, index) => index !== individualMealIndex,
					);
					...
					if (updatedMeals.length === 0) {
						...
						updateDateData({
							mealData: {
								...currentData.mealData,
								[mealType]: currentData.mealData[mealType].filter(
									(meal) => meal.id !== originalMealId,
								),
							},
						});
						...
						const updatedMeal = {
							...
							fat:
								Math.round(
									updatedMeals.reduce((sum, meal) => sum + (meal.fat || 0), 0) *
										10,
								) / 10,
							carbs:
								Math.round(
